...
# ╔═╡ 76e5a43c-bef4-11ef-3947-7f7700241caf
begin
	using CSV
	using Glob
	using Query
	...
	using Statistics
	using Logging
	# import Pkg; Pkg.add("SimpleMovingAverage")
	# using SimpleMovingAverage
end

# ╔═╡ 639c1b25-615b-4318-9225-67bbf5396ace
md"""
!!! info "Utility Functions"
"""

# ╔═╡ 733d3485-f32c-48b0-ada2-4222a3b6965b
"""
begin
    function ffill!(vec)
    ...
    function pointwise_lag(
        ...
        for i in 1:m
            ...
            for j in search_start:search_end
                ...
                distance, _ = dtw(x_slice, y_slice)
        ...
        time_lag = map(t -> abs(t) > max_lag_seconds ? NaN : t, time_lag)

        return time_lag
    end

    function filter_large_lags(time_lag, threshold::Float64=5.0)
        return map(t -> abs(t) > threshold ? NaN : t, time_lag)
    end
end
"""

# ╔═╡ 66cc9b2a-fe3a-459d-a198-5c0419392143
begin
    function ffill!(vec)
    ...
    function compute_irt(DF)
        ...
        for u in 1:n_vals
            ...
            # Ensure reaction time is bounded
            user_irt[u] = clamp(irt, 0, 5.0)  # Ensure it never goes beyond 5s or below 0
        end
    ...
    function pointwise_lag(
        x::Vector{Float64},  # Stimulus positions
        y::Vector{Float64},  # User positions
        sampling_frequency::Int64,
        max_lag_seconds::Float64=5.0
    )
        max_lag = Int(ceil(sampling_frequency * max_lag_seconds))
        ...
        for i in 1:m
            ...
            for j in search_start:search_end
                # Slicing vectors to compare for shape matching
                x_slice = x[max(1, i-1):min(m, i+1)]
                y_slice = y[max(1, j-1):min(n, j+1)]
        ...
        # Ensure lag is bounded by the max_lag_seconds threshold
        time_lag = lag ./ sampling_frequency
        time_lag = clamp.(time_lag, 0, max_lag_seconds)  # Clamp between 0 and max_lag_seconds

        return time_lag
    end
end

# ╔═╡ ad67ca3c-ca61-41a2-af45-c5c59cbaa95c
md"Getting IRT for CPT Participant Data"

# ╔═╡ 09ad8f43-5886-41fe-9877-94997cba0249
function process_files_with_irt(flist, outpath="/Users/danielgarcia-barnett/Desktop/Coding/cpCST_data_analysis/data/reprocessed_cpt_irt_data/", overwrite::Bool=false)
    for (i, fil) in enumerate(flist)
        filname = split(fil, '/')[end]
        outfile = outpath * "proc_" * filname
        
        # Check if the output file exists
        if isfile(outfile) && !overwrite
            @info "File $outfile already exists. Skipping..."
            continue
        end

        @info "Processing file $i of $(length(flist)): $filname"
        ...
        @info "Finished processing $filname, saved as $outfile"
    end
    
    return nothing
end

# ╔═╡ 11f0fec5-63c1-45ac-a5e4-759a8f74da9d
flist = Glob.glob("*CPT_run*.csv", "/Users/danielgarcia-barnett/Desktop/Coding/cpCST_data_analysis/data/raw_cpt_data")

# ╔═╡ 4e126923-0663-427e-8c70-9cd3e44ad835
begin
	ptp_list = ["M10929927", "M10992053", "M10913260", "M10917604", "M10970400", "M10952753", "M10909540", "M10984509", "M10958986", "M10945575", "M10943691", "M10966987", "M10942846", "M10905521", "M10970636"]
	
	# Filter the list to only include files containing any of the participants
	flist_filt = filter(file -> any(ptp -> occursin(ptp, file), ptp_list), flist)
	
	# Alternatively, using a comprehension
	flist_filt_comprehension = [file for file in flist if any(ptp -> occursin(ptp, file), ptp_list)]
end

# ╔═╡ 6ccf7dd0-e91b-43d5-a0b4-e11566ad1246
process_files_with_irt(flist)
...
manifest_format = "2.0"
project_hash = "7fdc422e0ef1b9238584aed1b1ab7752e356e0df"
